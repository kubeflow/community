# Kubeflow Outreach Committee

The Kubeflow Outreach Committee (KOC) is a committee dedicated to fostering growth, engagement, and community outreach for the Kubeflow project.

The [charter](charter.md) defines the scope and governance of the KOC.

## Members

Seats on the Outreach Committee are held by an individual, not by their employer.

The current membership of the committee is (listed alphabetically by first name):

| Name                     | Organization | GitHub                                                     |
| ------------------------ | ------------ | ---------------------------------------------------------- |
| Stefano Fioravanzo       | Canonical    | [StefanoFioravanzo](https://github.com/StefanoFioravanzo/) |
| Valentina Rodriguez Sosa | Red Hat      | [varodrig](https://github.com/varodrig/)                   |
| Tarek Abouzeid           | Telia        | [tarekabouzeid](https://github.com/tarekabouzeid/)         |

### Lead Members

| Name     | Organization | GitHub                                        | Role              |
| -------- | ------------ | --------------------------------------------- | ----------------- |
| Yash Pal |              | [yashpal2104](https://github.com/yashpal2104) | Social Media Lead |

## Meetings

The KOC currently meets monthly. Meetings are open to the public and held online.

- Last Friday every month at 8:00AM PST (Pacific Time)
- [Meeting notes](https://docs.google.com/document/d/1IblkZ0z61mjpSccIK7qDaMqUbFADZiIoOX9g8uDq_qc)
- [YouTube playlist recordings](https://www.youtube.com/@KubeflowCommunity)

## Contact

- Slack: [#kubeflow-outreach-distribution](https://cloud-native.slack.com/archives/C078ZMRQPB6)
- Open community [issue/PR](https://github.com/kubeflow/community/issues/new)
- GitHub Team: [@kubeflow-outreach-committee](https://github.com/orgs/kubeflow/teams/kubeflow-outreach-committee)
