# KEP-939: Infrastructure Management Process for CNCF Kubeflow OCI Tenancy
...
The OCI tenancy primarily supports development and testing use cases, including
environments that may not be possible with official CNCF infrastructure.
...
- **Introduce Default Time-Bound Allocations & Leases**
  Provide a scalable structure leveraging leases that allows the OCI tenancy to
  support Kubeflow's continued growth without creating operational ambiguity or
  "zombie" clusters.
...
- **Replacing Official CNCF Infrastructure or CI**
  The OCI tenancy may support temporary or experimental workloads, but this KEP
  does not aim to replace CNCF-managed infrastructure.
...
proposes an open discussion on whether the team handling OCI infrastructure
should operate as a sub-group within the existing Kubeflow Community
Distribution team (given the shared tooling and deployment standards) or be
formed as a separate, specialized Infrastructure Management team.
...
environment on OCI using GitOps. Instead of starting from scratch, they
reference the open-sourced community Terraform configurations and follow the
newly minted Kubeflow Field Operations Guide to securely deploy their cluster.
...
2. **Review & Approval:**
   Requests undergo review by KSC members, WG leads, and OCI administrators.
   Formal approval requires at least one `/approve` comment from an authorized
   maintainer.
...
Additionally, proactive Cost Alerts will be implemented at the OCI compartment
level. If a cluster's usage spikes or the overall tenancy approaches a specific
...
To ensure accountability, all resources within the OCI tenancy must adhere to a
mandatory tagging schema via Terraform:
...
2. **Review Workflow Dry-Run**: Simulate an infrastructure request with a dummy
   issue to verify the notification flow to designated OCI admins and KSC
   members.
3. **Audit and Cleanup Dry-Run**: Perform a manual audit of existing resources
   ...
   successfully queried in the OCI Console.

### Implementation Phases

#### Alpha Phase

- OCI administrators will execute resource provisioning through manual Terraform
  workflows.
...
- **Administrative Overhead**: Introduces manual verification steps for WG leads
  and OCI admins to review issues, verify tags, and audit lease renewals.
