# Kubeflow AI Reference Platform 26.03

### Links

- [Release Team](release-team.md)
- [Calendar](https://www.kubeflow.org/docs/about/community/#kubeflow-community-meetings)
- [Meeting Notes / Recordings](https://docs.google.com/document/d/1OUWifNf7hZ_QH3m1J3zjSUUk11yKUZnvom1BS2KxrUQ)
- [Project Board (Private)](https://github.com/orgs/kubeflow/projects/84)
- Contact
  - [#kubeflow-platform](https://cloud-native.slack.com/archives/C073W572LA2) on slack
  - [#kubeflow-release-team](https://cloud-native.slack.com/archives/C0ACZPHKR9N) on slack
  - [@release-team](https://github.com/orgs/kubeflow/teams/release-team) on GitHub

### TL;DR

The 26.03 release cycle is proposed as follows

**Note:** Following the successful prioritization of our graduation milestone, the standard release cycle has now been reinstated. This shorter-term release is crucial for integrating the significant updates and feature work contributed by all working groups over the recent months. Shorter release cycle due to first release to follow CalVer.

- **Monday, Jan 26th 2026** - Release team is formed
- **Monday, Jan 26th 2026** - Release cycle begins
- **Monday, Mar 2nd 2026** - Feature freeze, manifests sync starts, RC.1 is cut
- **Monday, Mar 16th 2026** - Distribution testing ends, pre-release bug fixing stage starts
- **Monday, Mar 23 2026** - Pre-release bug fixing stage ends
- **Monday, Mar 23 2026** - Docs update ends
- **Monday, Mar 23 2026** - Kubeflow v26.03 Released
