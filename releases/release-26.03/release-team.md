# Kubeflow AI Reference Platform 26.03 Release Team

| **Role** | **Name** (**GitHub / Slack ID**) |
|----------|----------------------------------|
| Release Manager |Tarek Abouzeid (GitHub: [@tarekabouzeid](https://github.com/tarekabouzeid) / Slack: `Tarek Abouzeid`)|
| Release Manager  Shadows | Dominik Kawka (GitHub: [@dominikkawka](https://github.com/dominikkawka) / Slack: `Dominik Kawka`) <br> Alok Dangre (GitHub: [@alokdangre](https://github.com/alokdangre) / Slack: `Alok Dangre`) <br>  Dhanisha Phadate (GitHub: [@dhanishaphadate](https://github.com/dhanishaphadate) / Slack: `Dhanisha Phadate`) <br> Milind Dethe (GitHub: [@milinddethe15](https://github.com/milinddethe15) / Slack: `Milind`)|
| Product Manager | Dhanisha Phadate (GitHub: [@dhanishaphadate](https://github.com/dhanishaphadate) / Slack: `Dhanisha Phadate`)|
| AutoML and Training WG Liaison(s) | Anya Kramar (GitHub: [@kramaranya](https://github.com/kramaranya) / Slack: `Anya`)|
| Notebooks WG Liaison(s) | Andy Stoneberg (GitHub: [@andyatmiami](https://github.com/andyatmiami) / Slack: `Andy Stoneberg`)
| Platform WG Liaison(s) | Tarek Abouzeid (GitHub: [@tarekabouzeid](https://github.com/tarekabouzeid) / Slack: `Tarek Abouzeid`) |
| Pipelines WG Liaison(s) | TBD|
| Spark Operator WG Liaison(s) | Milos Grubjesic (GitHub: [@milosjava](https://github.com/milosjava) / Slack: `Milos Grubjesic` ) |
| Model Registry WG Liaison(s) | Matteo Mortari (GitHub: [@tarilabs](https://github.com/tarilabs) / Slack: `Matteo Mortari` ) <br> Adysen Rothman (GitHub: [adysenrothman](https://github.com/adysenrothman) / Slack: `Adysen Rothman`) | 
| KServe Liaison(s) | Vraj Bhatt (GitHub: [@vrajjbhatt](https://github.com/vrajjbhatt) / Slack: `Vraj Bhatt`) |

- The definition of the release team roles and responsibilities can be found at [release handbook](../kubeflow-ai-reference-platform-release-handbook.md)
- The timeline for the release can be found [here](README.md).

